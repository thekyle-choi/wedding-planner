...
// 년도별 전체 소득 계산
export function calculateYearlyIncome(yearlyData: YearlyIncomeData): YearlyCalculatedIncome {
  const jkCalculation = calculatePersonIncome(
    yearlyData.jkData,
    yearlyData.template.incomeItems,
    yearlyData.template.deductionItems
  )
...
export function createEmptyCalculation(personId: 'jk' | 'sj', year: number): CalculatedPersonIncome {
