...
export function calculatePersonIncome(
  ...
  // 총 비과세소득 계산
  const totalTaxExemptIncome = calculateTotalTaxExemptIncome(allItems, personData.incomeValues)
  ...
  // 과세표준 (근로소득금액 - 기본공제)
  const taxableStandard = Math.max(0, taxableIncomeAfterDeduction - BASIC_DEDUCTION)
