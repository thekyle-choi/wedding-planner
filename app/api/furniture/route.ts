...
const FURNITURE_KEY = "wedding:furniture"
...
export async function GET() {
  try {
    const data = await redis.get<any[]>(FURNITURE_KEY)
    const list = Array.isArray(data) ? data : []
    
    const normalized: FurnitureCategory[] = list
      .map((category) => {
        const items: FurnitureItem[] = Array.isArray(category?.items)
    ...
    console.error("Failed to get furniture categories:", error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const now = Date.now()
    const normalized: FurnitureCategory[] = payload.map((category) => {
      const items: FurnitureItem[] = Array.isArray(category?.items)
        ? category.items.map((item: any) => ({
            id: typeof item?.id === "string" ? item.id : crypto.randomUUID(),
    ...
    await redis.set(FURNITURE_KEY, normalized)
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const existing = await redis.get<FurnitureCategory[]>(FURNITURE_KEY)
    const categories = Array.isArray(existing) ? existing : []

    if (typeof categoryId === "string" && !itemId) {
      // Delete entire category
      const filtered = categories.filter((c) => c.id !== categoryId)
      await redis.set(FURNITURE_KEY, filtered)
      ...
      await redis.set(FURNITURE_KEY, updated)
      return NextResponse.json({ success: true })
    } else {
      return NextResponse.json({ error: "invalid_request" }, { status: 400 })
    }
  } catch (error) {
    console.error("Failed to delete furniture data:", error)
